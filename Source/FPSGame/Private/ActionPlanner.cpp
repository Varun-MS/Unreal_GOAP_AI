...
{
	return std::find_if(begin(m_openList), end(m_openList), [&](const NodeRecord& n) { return n.pWorldState == i_pWorldState; });
}

bool UActionPlanner::IsContainedInClosedList(TWeakObjectPtr<UWorldState> i_pWorldState)
{
	if (std::find_if(begin(m_closedList), end(m_closedList), [&](const NodeRecord& n) { return n.pWorldState == i_pWorldState; }) == end(m_closedList)) {
		return false;
	}
	return true;
}
